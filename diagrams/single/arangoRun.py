...
for threads in "1","2","4","16","48","96":
  for workload in "B", "C", "E", "A":
    for ops in "10000", "100000", "300000", "1000000", "1500000":
      a = f"./drive/MyDrive/arangodb/arango_Workload{workload}/singleArango_wl"
      b = "_run_"
      c = "Thr_"
      d = "Ops.txt"
      ...
      try:
        ...
        w = content[content.find("[OVERALL], Throughput(ops/sec), ")+len("[OVERALL], Throughput(ops/sec), "):content.rfind("[SCAN], Operations,")]
        y = float(w)
      f.close()
      throughput[i].append(y)
      i= i + 1
  ...
  df.plot(x='Operations',
        kind='bar',
        stacked=False,
        title=f'Single Instance ArangoDB, {threads} Thread: Throughput-Operations Diagram')
...
for threads in "1","2","4","16","48","96":
  ...
  df.plot(x='Operations',
        kind='bar',
        stacked=False,
        title=f'Single Instance ArangoDB, {threads} Thread: Latency-Operations Diagram')
